...
func (file *fileStore) buildFile(ctx context.Context,name string) string {
	dir := filepath.Join(file.directory, name[len(name)-file.level:])
	if _,err := os.Stat(dir);err != nil && os.IsNotExist(err){
		os.Mkdir(dir, file.fileMode)
	}
	return filepath.Join(dir, name)
}

// Matching the filename prefix clears the file
func(file *fileStore) Flush(ctx context.Context, prefix string) error {
...
func(file *fileStore) Clear(ctx context.Context) error {
	return nil
}
